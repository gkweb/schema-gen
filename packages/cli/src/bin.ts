...
cli
  ...
  .action(
    async (
      ...
      options: { config?: string; input?: string; output?: string; watch?: boolean },
    ) => {
          ...
          fs.watch(specPath, async () => {
            ...
            debounceTimer = setTimeout(async () => {
              ...
              try {
                ...
                await gen.generate();
                console.log(
                  pc.green(`✓ Regenerated in ${(performance.now() - start).toFixed(0)}ms`),
                );
              } catch (err) {
...
// Validate command
cli.command('validate <spec>', 'Validate an OpenAPI specification').action(async (spec: string) => {
  try {
