...
export function reactQueryV5(config?: ReactQueryV5Config): Plugin {
  return {
    ...
    emit(ctx) {
      ...
      // Track formDataFn imports: key is "from|name", value is { from, name }
      const formDataImports = new Map<string, { from: string; name: string }>();
      const sections: string[] = [];

      // Build set of known enum names from AST for accurate enum detection
      ...
      for (const endpoint of ctx.ast.endpoints) {
        ...
        if (isQuery) {
          sections.push(generateQueryCode(endpoint, resolvedConfig, ctx.utils, typeImports, enumImports, knownEnums));
        }

        if (isMutation) {
          sections.push(
            generateMutationCode(endpoint, resolvedConfig, ctx.utils, typeImports, enumImports, knownEnums, formDataImports, ctx.log),
          );
        }
      }

      // Build final output with imports at top
      const imports = generateImports(resolvedConfig, typeImports, enumImports, formDataImports);
      const content = [
...
function generateImports(
  ...
  // Custom fetch import
  if (config.fetchFn) {
    lines.push(`import { ${config.fetchFn.name} } from '${config.fetchFn.from}';`);
  }
...
function buildMutationFn(
  ...
  if (config.fetchFn) {
    const fetchName = config.fetchFn.name;
    if (hasBody) {
