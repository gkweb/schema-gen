	...
	return {
		...
		emit(ctx) {
			...
			for (const endpoint of ctx.ast.endpoints) {
				...
				if (isQuery) {
					sections.push(
						generateQueryCode(endpoint, resolvedConfig, ctx.utils, typeImports),
					);
				}

				if (isMutation) {
					sections.push(
						generateMutationCode(endpoint, resolvedConfig, ctx.utils, typeImports),
					);
				}
			}
...
function generateUseMutationHook(
	...
	const mutationFn = buildMutationFn(endpoint, bodyType, hasPathParams, hasBody, pathParams, config);
...
function getEndpointParams(endpoint: EndpointNode): ParameterNode[] {
	return (endpoint.parameters ?? []).filter(
		(p) => p.location === 'path' || p.location === 'query',
	);
}
...
function buildFetchCall(
	...
	if (hasQueryParams) {
		...
		for (const param of queryParams) {
			lines.push(`      if (p.${param.name} !== undefined) searchParams.append('${param.name}', String(p.${param.name}));`);
		}
