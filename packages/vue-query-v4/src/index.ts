  ...
  return {
    ...
    emit(ctx) {
      ...
      for (const endpoint of ctx.ast.endpoints) {
        ...
        if (isMutation) {
          sections.push(
            generateMutationCode(endpoint, resolvedConfig, ctx.utils, typeImports, formDataImports, ctx.log),
          );
        }
      }

      // Build final output with imports at top
      const imports = generateImports(resolvedConfig, typeImports, formDataImports);
      const content = [
