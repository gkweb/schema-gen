...
export interface VueQueryV4Config {
   ...
   * function before passing it to Vue Query. Equivalent to orval's
   * `output.override.query.queryOptions`.
   *
   * The function is invoked with `(options, opContext)` where
   * `opContext = { operationId, method, path }`.
   */
  queryOptions?: {
    from: string;
    name: string;
  ...
  return {
    ...
    emit(ctx) {
      ...
      return groups.map(({ fileName, endpoints }) =>
        renderGroup(fileName, endpoints, resolvedConfig, ctx.utils, ctx.log),
      );
    },
  };
}
 ...
 *
 * Mirrors the react-query-v5 plugin's grouping for symmetry.
 */
...
/**
 * Render one Vue Query module from a list of endpoints.
 */
function renderGroup(
  ...
  // fetchFn / queryOptions / mutationOptions imports referenced by emit.
  ...
  for (const endpoint of endpoints) {
    ...
    if (isQuery) {
      sections.push(generateQueryCode(endpoint, config, utils, typeImports, extraImports));
    }
    if (isMutation) {
      sections.push(
        generateMutationCode(endpoint, config, utils, typeImports, formDataImports, extraImports, log),
      );
    }
  }

  const imports = generateImports(config, typeImports, formDataImports, extraImports);
  const content = [
 ...
 */\n`;
  ...
  // When a queryOptions wrapper is configured, every hook routes its options
  // object through `${wrapper}(options, opContext)` before handing it to
  // Vue Query. This mirrors orval's `output.override.query.queryOptions`.
...
function generateMutationCode(
  ...
  if (isFormData && formDataFnConfig === undefined) {
    logger.warn(
      `[vue-query-v4] Endpoint "${endpoint.operationId ?? endpoint.id}" uses multipart/form-data but no formDataFn is configured. ` +
 ...
 */\n`;
  ...
  // When a mutationOptions wrapper is configured, the whole options object
  // (including the generated `mutationFn` and any user-supplied options)
  // gets routed through `${wrapper}(opts, opContext)` first.
...
function getRequestBodyType(endpoint: EndpointNode, typeImports: Set<string>): string {
  ...
  if (formDataContent?.typeRef) {
    return extractTypeName(formDataContent.typeRef, typeImports);
  }
